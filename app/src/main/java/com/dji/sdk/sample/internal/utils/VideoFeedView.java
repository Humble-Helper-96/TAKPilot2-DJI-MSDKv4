...
public class VideoFeedView extends SurfaceView {
    ...
    private void init(Context context) {
        ...
        videoDataListener = new VideoFeeder.VideoDataListener() {
            ...
            public void onReceive(byte[] videoBuffer, int size) {
                ...
                // TAKPilot2 Go diagnostic (Phase 4): confirm raw video bytes are actually
                // arriving from VideoFeeder, independent of whether the codec renders them.
                long n = frameCount.incrementAndGet();
                if (n == 1 || n % 150 == 0) {
                    android.util.Log.d(TAG, "onReceive: frame #" + n + " size=" + size + "B");
                }
    ...
    protected void onDetachedFromWindow() {
        ...
        // TAKPilot2 Go: do NOT call VideoFeeder.getInstance().destroy() here. This class was
        // written for one-shot stock-sample demo screens; our flight screen is visited
        // repeatedly (Home <-> Flight), and destroying the shared VideoFeeder singleton on
        // every detach broke video on the second and subsequent visits (codec initializes
        // fine, but no real frame data ever arrives). Removing a video data listener from a
        // feed we don't own is enough cleanup here.
    }
}
